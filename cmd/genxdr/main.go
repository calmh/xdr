...
func (o {{.TypeName}}) EncodeXDRInto(xw *xdr.Writer) (int, error) {
	{{range $fi := .Fields}}
		{{if $fi.IsSlice}}
			{{template "encodeSlice" $fi}}
		{{else}}
			{{template "encodeValue" $fi}}
		{{end}}
	{{end}}
...
{{define "encodeValue"}}
	{{if ne .Convert ""}}
		xw.Write{{.Encoder}}({{.Convert}}(o.{{.Name}}))
	{{else if .IsBasic}}
		{{if ge .Max 1}}
			if l := len(o.{{.Name}}); l > {{.Max}} {
				return xw.Tot(), xdr.ElementSizeExceeded("{{.Name}}", l, {{.Max}})
			}
		{{end}}
		xw.Write{{.Encoder}}(o.{{.Name}})
	{{else}}
		_, err := o.{{.Name}}.EncodeXDRInto(xw)
		if err != nil {
			return xw.Tot(), err
		}
	{{end}}
...
{{define "encodeSlice"}}
	...
	xw.WriteUint32(uint32(len(o.{{.Name}})))
	for i := range o.{{.Name}} {
		{{if ne .Convert ""}}
			xw.Write{{.Encoder}}({{.Convert}}(o.{{.Name}}[i]))
		{{else if .IsBasic}}
			xw.Write{{.Encoder}}(o.{{.Name}}[i])
		{{else}}
			_, err := o.{{.Name}}[i].EncodeXDRInto(xw)
			if err != nil {
				return xw.Tot(), err
			}
		{{end}}
...
func (o *{{.TypeName}}) DecodeXDRFrom(xr *xdr.Reader) error {
	{{range $fi := .Fields}}
		{{if $fi.IsSlice}}
			{{template "decodeSlice" $fi}}
		{{else}}
			{{template "decodeValue" $fi}}
		{{end}}
	{{end}}
	return xr.Error()
}
...
{{define "decodeValue"}}
	{{if ne .Convert ""}}
		o.{{.Name}} = {{.FieldType}}(xr.Read{{.Encoder}}())
	{{else if .IsBasic}}
		{{if ge .Max 1}}
			o.{{.Name}} = xr.Read{{.Encoder}}Max({{.Max}})
		{{else}}
			o.{{.Name}} = xr.Read{{.Encoder}}()
		{{end}}
	{{else}}
		(&o.{{.Name}}).DecodeXDRFrom(xr)
	{{end}}
{{end}}

{{define "decodeSlice"}}
	_{{.Name}}Size := int(xr.ReadUint32())
	if _{{.Name}}Size < 0 {
	...
	for i := range o.{{.Name}} {
		{{if ne .Convert ""}}
			o.{{.Name}}[i] = {{.FieldType}}(xr.Read{{.Encoder}}())
		{{else if .IsBasic}}
			{{if ge .Submax 1}}
				o.{{.Name}}[i] = xr.Read{{.Encoder}}Max({{.Submax}})
			{{else}}
				o.{{.Name}}[i] = xr.Read{{.Encoder}}()
			{{end}}
		{{else}}
			(&o.{{.Name}}[i]).DecodeXDRFrom(xr)
		{{end}}
	}
{{end}}
`
